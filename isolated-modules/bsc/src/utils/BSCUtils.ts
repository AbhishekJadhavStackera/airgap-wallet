...
export class BSCUtils {
  public static toHex(value: any): string {
    ...
    if (BSCUtils.isObject(value) && !BSCUtils.isBigNumber(value) /* && !BSCUtils.isBN(value)*/) {
      return BSCUtils.utf8ToHex(JSON.stringify(value))
    }

    // if its a negative number, pass it through numberToHex
    if (typeof value === 'string') {
      if (value.indexOf('-0x') === 0 || value.indexOf('-0X') === 0) {
        return BSCUtils.numberToHex(value)
      } else if (value.indexOf('0x') === 0 || value.indexOf('0X') === 0) {
        return value
      } else if (!isFinite(Number(value))) {
        return BSCUtils.utf8ToHex(value)
      }
    }
  ...
  public static sha3(value: any): string | null {
    ...
    if (BSCUtils.isHexStrict(value) && /^0x/i.test(value.toString())) {
      valueInBytes = BSCUtils.hexToBytes(value)
    }

    const hash: string = createKeccakHash('keccak256').update(valueInBytes).digest('hex')
    const returnValue: string = `0x${hash}`

    if (returnValue === BSCUtils.SHA3_NULL_S) {
      return null
    } else {
      return returnValue
    }
  ...
  private static checkAddressChecksum(value: string): boolean {
    ...
    let addressHash: string | null = BSCUtils.sha3(address.toLowerCase())
  ...
  private static isAddress(value: string): boolean {
    if (!/^(0x)?[0-9a-f]{40}$/i.test(value)) {
      ...
      return BSCUtils.checkAddressChecksum(value)
    }
  }
